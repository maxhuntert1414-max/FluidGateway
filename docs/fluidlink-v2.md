...
sharing and freezes their hashes. A unique context SHA-256 binds the nonce,
peer process evidence, target/hook hashes, pair/phase, transfer shape, action,
...
FluidLink is local user-space IPC without hostile-peer authentication. It is
advisory by default and does not turn discrete memory into unified memory. The
single v0.15 owned-lab consumer may authorize an attempt at action bit 8 only
after the exact sequence above; it cannot bypass native content proof. Every
actuation still requires owned-target opt-in, provenance, bounded action and
