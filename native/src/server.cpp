...
bool send_frame(SOCKET socket, std::span<const std::uint8_t> bytes) {
    ...
    while (!bytes.empty()) {
        if (!ready(socket, POLLWRNORM, deadline))
            return false;
        const auto count = send(
            socket, reinterpret_cast<const char*>(bytes.data()), static_cast<int>(bytes.size()), 0);
        if (count == SOCKET_ERROR && WSAGetLastError() == WSAEWOULDBLOCK)
            continue;
        if (count <= 0)
            return false;
    ...
    try {
        ...
        while (!stopping.load() && !session.closed) {
            ...
            if (!receive(connected,
                         std::span(wire).first(prefix_count),
                         Clock::now() + std::chrono::seconds(first ? 1 : 30)))
                break;
            const auto deadline = Clock::now() + std::chrono::seconds(2);
            if (first && (wire[0] != 'F' || wire[1] != 'L' || wire[2] != 'N' || wire[3] != 'K' ||
                          wire[4] != 2))
                break;
            first = false;
            if (!receive(connected, std::span(wire).subspan(prefix_count), deadline))
                break;
            const auto size = payload_size(wire);
            wire.resize(header_size + size);
            if (!receive(connected, std::span(wire).subspan(header_size), deadline))
                break;
            const auto response = session.process(decode_frame(wire));
            if (!send_frame(connected, encode_frame(response)))
                break;
        }
    } catch (...) {
...
int main(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; i += 2) {
        ...
        if (key == "--host") {
            ...
            if (parsed.ec != std::errc{} || parsed.ptr != value.data() + value.size() || !port ||
                port > 65535)
                return 2;
        } else
            return 2;
    ...
    try {
        ...
        BOOL exclusive = TRUE;
        if (setsockopt(listener.value,
                       SOL_SOCKET,
                       SO_EXCLUSIVEADDRUSE,
                       reinterpret_cast<const char*>(&exclusive),
                       sizeof(exclusive)))
            throw std::runtime_error("Cannot protect listener.");
        sockaddr_in address{};
        address.sin_family = AF_INET;
        address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
        address.sin_port = htons(static_cast<u_short>(port));
        if (bind(listener.value, reinterpret_cast<const sockaddr*>(&address), sizeof(address)) ||
            listen(listener.value, 16))
            throw std::runtime_error("Cannot bind/listen; the port may already be in use.");
        u_long nonblocking = 1;
        if (ioctlsocket(listener.value, FIONBIO, &nonblocking))
            throw std::runtime_error("Cannot set nonblocking mode.");
        if (!SetConsoleCtrlHandler(stop_handler, TRUE))
            throw std::runtime_error("Cannot register shutdown handler.");
