...
Resource::Resource(std::uint8_t layer,
                   const std::vector<std::string>& names,
                   std::pmr::memory_resource* allocator)
    : aliases(allocator), memory(layer) {
    for (const auto& name : names)
        aliases.emplace_back(name);
}
void State::begin_frame(std::uint64_t frame) {
    if (current_frame_)
        throw Error(11, "A frame is already open.");
    current_frame_ = frame;
}
void State::end_frame(std::uint64_t frame) {
    if (current_frame_ != frame)
        throw Error(11, "Frame does not match the open frame.");
    current_frame_.reset();
}
void State::end_session() const {
    if (current_frame_)
        throw Error(11, "Cannot end session with an open frame.");
}
static bool aliases_intersect(const Resource& a, const Resource& b) {
    if (a.memory != b.memory)
        return false;
    for (const auto& name : a.aliases)
        if (std::find(b.aliases.begin(), b.aliases.end(), name) != b.aliases.end())
            return true;
    return false;
}
void State::register_resource(std::string_view id,
                              std::uint8_t memory,
                              const std::vector<std::string>& aliases) {
    if (resources_.contains(id))
        throw Error(11, "Duplicate resource id.");
    if (resources_.size() == resource_limit)
        throw Error(11, "Resource limit reached; reconnect.", true);
...
void State::invalidate(std::string_view id) {
    ...
    for (const auto& [name, resource] : resources_) {
        ...
        if (&resource != affected[0])
            remaining[remaining_count++] = &resource;
    }
    ...
    for (auto& [name, resource] : resources_) {
        ...
        if (std::find(affected.begin(), affected.begin() + count, &resource) !=
                affected.begin() + count ||
            (source != resources_.end() &&
             std::find(affected.begin(), affected.begin() + count, &source->second) !=
                 affected.begin() + count))
            resource.copy = nullptr;
    }
}
...
Decision State::process(Operation op) {
    if (operations_.contains(std::string_view(op.id)))
        throw Error(11, "Duplicate operation id.");
    if (operations_.size() == operation_limit)
        throw Error(11, "Operation limit reached; reconnect.", true);
    if (!op.frame)
        op.frame = current_frame_;
    ...
    if ((op.type == 1 || op.type == 4) && source != resources_.end() &&
        target != resources_.end()) {
        ...
        if (op.source == op.target)
            ...
            retained = source->second.copy;
                   ...
                   previous->bytes == op.bytes && previous->frame == op.frame &&
                   previous->queue == op.queue) {
            decision.opcode = 2;
            retained = previous;
        }
    } else if (op.type == 2) {
        const bool all_removed =
            !op.dependencies.empty() &&
            std::all_of(op.dependencies.begin(), op.dependencies.end(), [this](const auto& id) {
                const auto found = operations_.find(std::string_view(id));
                return found != operations_.end() && found->second.removed;
            });
        if (all_removed)
            decision.opcode = 4;
        else if (!op.microseconds && op.dependencies.empty())
            decision.opcode = 5;
    } else if (op.type == 3 && target != resources_.end()) {
        const auto previous = target->second.allocation;
        std::transform(op.reason.begin(), op.reason.end(), op.reason.begin(), [](unsigned char c) {
            return static_cast<char>(std::tolower(c));
        });
        const bool transient = op.reason.find("transient") != std::string::npos ||
                               op.reason.find("scratch") != std::string::npos ||
                               op.reason.find("temporary") != std::string::npos ||
    ...
    auto [inserted, unused] =
        operations_.try_emplace(std::pmr::string(op.id, &budget), op, &budget);
    (void)unused;
    auto& stored = inserted->second;
    stored.eliminated = decision.opcode != 0;
    auto append = [&stored](std::string_view id) {
        if (std::find(stored.replacements.begin(), stored.replacements.end(), id) ==
            stored.replacements.end())
    ...
    for (const auto& dependency : op.dependencies) {
        ...
        if (found != operations_.end() && found != inserted && found->second.eliminated) {
            for (const auto& replacement : found->second.replacements)
                append(replacement);
        } else
            append(dependency);
    }
    if (stored.eliminated) {
        ...
        if (op.type != 2)
            decision.saved_bytes = op.bytes;
    } else {
        if (op.type == 1 || op.type == 3 || op.type == 4 || op.type == 6 || op.type == 7)
            invalidate(op.target);
        if (target != resources_.end()) {
            if (op.type == 1 || op.type == 4)
                target->second.copy = &stored;
            if (op.type == 3)
                target->second.allocation = &stored;
        }
    }
    return decision;
}
