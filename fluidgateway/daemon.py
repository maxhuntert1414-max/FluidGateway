...
from .daemon_control_execution import (
    ...
    dry_run_runtime_daemon_control_plan,
)
...
class RuntimeDaemonReport:
    ...
    daemon_control_execution_policy: str
    daemon_control_execution_blocked_count: int
    daemon_control_execution_executed_count: int
    ...
    native_backend_readiness_policy: str
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "daemon_arbitration_policy": self.daemon_arbitration_policy,
            "daemon_arbitration_blocked_count": (
                self.daemon_arbitration_blocked_count
            ),
            ...
            "daemon_control_execution_policy": (
                self.daemon_control_execution_policy
            ),
            "daemon_control_execution_blocked_count": (
                self.daemon_control_execution_blocked_count
            ),
            "daemon_control_execution_executed_count": (
                self.daemon_control_execution_executed_count
            ),
            "native_backend_manifest_policy": (
                self.native_backend_manifest_policy
            ),
            "native_backend_manifest_ready_count": (
                self.native_backend_manifest_ready_count
            ),
            ...
            "native_backend_readiness_policy": (
                self.native_backend_readiness_policy
            ),
            "native_backend_readiness_ready_count": (
                self.native_backend_readiness_ready_count
            ),
            "native_backend_readiness_blocked_count": (
                self.native_backend_readiness_blocked_count
            ),
...
def run_runtime_daemon(
    ...
    daemon_arbitration_plan = build_runtime_daemon_arbitration_plan(
        final_state=previous_state,
    ...
    daemon_control_execution = dry_run_runtime_daemon_control_plan(
        daemon_control_plan,
    )
    native_backend_manifest = build_runtime_native_backend_manifest(
        control_execution=daemon_control_execution,
        native_backend_preflight=native_backend_preflight,
        host_snapshot=host_snapshot,
    )
    ...
    return RuntimeDaemonReport(
        ...
        daemon_arbitration_policy=daemon_arbitration_plan.arbitration_policy,
        daemon_arbitration_blocked_count=(
        ...
        daemon_control_execution_policy=(
            daemon_control_execution.execution_policy
        ),
        daemon_control_execution_blocked_count=(
            daemon_control_execution.blocked_count
        ),
        daemon_control_execution_executed_count=(
            daemon_control_execution.executed_count
        ),
        native_backend_manifest_policy=(
            native_backend_manifest.manifest_policy
        ),
        ...
        native_backend_readiness_policy=native_backend_readiness.readiness_policy,
        native_backend_readiness_ready_count=(
            native_backend_readiness.ready_observation_count
            + native_backend_readiness.advisory_ready_count
        ),
        native_backend_readiness_blocked_count=(
            native_backend_readiness.native_blocked_count
        ),
        native_backend_readiness_high_risk_count=(
