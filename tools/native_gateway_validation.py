...
def native_executable() -> Path:
    return Path(os.environ.get("FLUIDGATEWAY_NATIVE", ROOT / "native/build/Release/fluidgateway-native.exe"))
...
class Peer:
    def __init__(self, port: int, batch: bool = True, *, receive_buffer_bytes: int | None = None):
        ...
        try:
            self.welcome = self.request(1, wire.encode_hello_payload(
                client_name="native-validation", client_version="1",
    ...
    def request(self, opcode: int, payload: bytes = b"", subject: int = 0):
        self.sequence += 1
        request = wire.fluidlink_v2_request(opcode=opcode, sequence=self.sequence,
            payload=payload, subject_opcode=subject, session_id=self.session)
        ...
        if response is None or response.message_id != request.message_id or response.sequence != self.sequence:
    ...
    def batch(self, count: int = 129, **overrides):
        event = dict(batch_id=uuid4().hex, operation_count=count,
            operation_type="upload", queue="copy", source="ram", target="vram",
            size_bytes=4194304, cost_us=300, frame=0)
    ...
    def initialize(self):
        for response in (
            ...
            self.event(102, action="register", id="ram", kind="buffer", memory="ram", size_bytes=4194304),
            self.event(102, action="register", id="vram", kind="buffer", memory="vram", size_bytes=4194304),
        ):
...
def server(backend: str = "Native", executable: Path | None = None):
    ...
    command = ([str(executable), "serve-events"] if backend == "Native" else
               [sys.executable, "-u", "-m", "fluidgateway", "runtime", "serve-events"])
...
def process_metrics(pid: int) -> dict:
    ...
    try:
        ...
        if not psapi.GetProcessMemoryInfo(handle, ctypes.byref(counters), counters.cb) or not kernel.GetProcessTimes(
        ...
        if not kernel.QueryProcessCycleTime(handle, ctypes.byref(cycles)):
            raise ctypes.WinError(ctypes.get_last_error())
        return dict(private_bytes=counters.private, working_set_bytes=counters.working,
                    cpu_ns=cpu_ns, cpu_cycles=cycles.value)
    finally:
...
def distribution(values):
    ...
    return dict(count=len(ordered), p50=percentile(.5), p95=percentile(.95),
                p99=percentile(.99), maximum=max(ordered), mean=statistics.mean(ordered))
...
def soak(executable: Path, seconds: float, events: int, out: Path):
    ...
    samples = []
    next_sample = 0.0
    result = dict(schema="fluidgateway-native-soak-v1", complete=False, seconds_required=seconds,
        minimum_operations=events, executable_sha256=hashlib.sha256(executable.read_bytes()).hexdigest())
    with server(executable=executable) as (process, port):
        while time.monotonic() - started < seconds or count < events:
            with Peer(port) as peer:
                ...
                for index in range(16):
                    ...
                    decisions = wire.decode_operation_batch_decision_payload(response.payload)["decisions"]
            ...
            if elapsed >= next_sample:
                samples.append(dict(elapsed_seconds=elapsed, operations=count, sessions=sessions,
                                    **process_metrics(process.pid)))
                result.update(elapsed_seconds=elapsed, operations=count, sessions=sessions, samples=samples)
        ...
        result.update(complete=True, elapsed_seconds=time.monotonic() - started,
                      operations=count, sessions=sessions, final_metrics=process_metrics(process.pid))
    ...
    result["memory_plateau_passed"] = bool(stable) and max(stable) <= min(stable) * 1.10 + 1024 * 1024
    write_result(out, result)
    return result
...
def benchmark(executable: Path, pairs: int, out: Path):
    ...
    with server("Python") as (python, python_port), server("Native", executable) as (native, native_port):
        ...
        for index in range(pairs + 5):
            for backend in (("Python", "Native") if index % 2 == 0 else ("Native", "Python")):
                ...
                if index >= 5:
                    records[backend].append(dict(pair=index - 5, authorization_us=elapsed,
                        cpu_ns=after["cpu_ns"] - before["cpu_ns"], private_bytes=after["private_bytes"],
                        cpu_cycles=after["cpu_cycles"] - before["cpu_cycles"],
                        roundtrip_us=roundtrips))
    summaries = {}
    for backend, runs in records.items():
        summaries[backend] = dict(authorization_us=distribution([r["authorization_us"] for r in runs]),
    ...
    passed = pairs >= 30 and all(native["authorization_us"][p] <= baseline["authorization_us"][p]
        for p in ("p95", "p99")) and 0 < native["cpu_cycles_per_operation"] < baseline["cpu_cycles_per_operation"]
    result = dict(schema="fluidgateway-native-comparison-v1", pairs=pairs, warmup_pairs=5,
        native_sha256=hashlib.sha256(executable.read_bytes()).hexdigest(),
        summaries=summaries, samples=records, comparison_gate_passed=passed,
...
def main():
    ...
    result = (soak(args.native.resolve(), args.seconds, args.events, args.out) if args.mode == "soak" else
              benchmark(args.native.resolve(), args.pairs, args.out))
    print(json.dumps({k: v for k, v in result.items() if k not in ("samples", "summaries")}, indent=2))
    return 0 if result.get("memory_plateau_passed", result.get("comparison_gate_passed", False)) else 1
