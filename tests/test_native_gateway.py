...
def setup_events():
    return [event(100, action="begin", id="test"), event(101, action="begin", frame=0),
        event(102, id="ram", memory="ram"), event(102, id="vram", memory="vram")]


def op(name, **overrides):
    payload = dict(id=name, operation_type="upload", source="ram", target="vram",
                   queue="copy", cost_us=320, size_bytes=1048576)
    payload.update(overrides)
    return event(103, **payload)


@unittest.skipUnless(NATIVE.exists() and TEST_BINARY.exists(), "Build the native Gateway or set FLUIDGATEWAY_NATIVE")
class NativeGatewayTests(unittest.TestCase):
    def compare(self, events, batch=False):
        protocol = wire.FluidLinkV2ServerSession(server_name="fluidgateway", server_version="0.68.0")
        adapter = RuntimeAdapterSession()
        hello = wire.encode_hello_payload(client_name="test", client_version="1",
            contract_digest=wire.FLUIDLINK_V2_BATCH_CONTRACT_DIGEST if batch else wire.FLUIDLINK_V2_CONTRACT_DIGEST,
            requested_capabilities=wire.FLUIDLINK_V2_BATCH_CAPABILITIES if batch else wire.FLUIDLINK_V2_CAPABILITIES,
            required_capabilities=wire.FLUIDLINK_V2_BATCH_REQUIRED_CAPABILITIES if batch else wire.FLUIDLINK_V2_REQUIRED_CAPABILITIES)
        ...
        for sequence, (subject, payload) in enumerate(events, 2):
            request = wire.fluidlink_v2_request(opcode=10, sequence=sequence, session_id=IDENTITY,
                                              payload=payload, subject_opcode=subject)
            requests.append(request)
            expected.append(protocol.process(request, adapter.process_event))
        result = subprocess.run([str(TEST_BINARY), "--replay"], text=True, capture_output=True,
            input="\n".join(wire.encode_fluidlink_v2_frame(r).hex() for r in requests), check=True, timeout=30)
        ...
        for index, (line, reference) in enumerate(zip(lines, expected)):
            ...
            self.assertEqual((actual.opcode, actual.subject_opcode, actual.decision_opcode, actual.ok),
                (reference.opcode, reference.subject_opcode, reference.decision_opcode, reference.ok), index)
    ...
    def test_golden_frame_roundtrips(self):
        ...
        result = subprocess.run([str(TEST_BINARY), "--roundtrip"], input="\n".join(encoded),
                                text=True, capture_output=True, check=True, timeout=10)
        self.assertEqual(result.stdout.splitlines(), encoded)

    def test_copy_write_and_dependency_parity(self):
        self.compare(setup_events() + [op("seed"), op("duplicate"),
            op("wait", operation_type="sync", depends_on=["duplicate"]),
            op("write", operation_type="compute", target="ram"), op("after-write"),
            op("self", target="ram"), op("orphan", operation_type="sync", depends_on=["self"]),
            op("unknown", source="missing", target="missing"),
            op("empty", operation_type="sync", cost_us=0),
            event(101, action="end", frame=0), event(100, action="end", id="")])

    def test_alias_release_resize_parity(self):
        self.compare([event(102, id="ram", memory="ram", aliases=["x"]),
            event(102, id="alias", memory="ram", aliases=["x", "y"]),
            event(102, id="alias2", memory="ram", aliases=["y"]),
            event(102, id="vram", memory="vram"), op("seed"),
            op("write", operation_type="draw", target="alias2"), op("fresh"),
            op("aliased-copy", source="ram", target="alias"),
            event(102, action="release", id="vram"), event(102, id="vram", memory="vram"),
            op("reregistered"), op("alloc", operation_type="allocate", reason="temporary"),
    ...
    def test_frame_queue_and_unknown_write_parity(self):
        self.compare(setup_events() + [op("a"), op("b", queue="graphics"), op("c", frame=1),
            op("d", operation_type="draw", target="unknown"), op("e", frame=1),
            event(101, action="end", frame=3), event(100, action="end", id=""),
            event(101, action="end", frame=0), event(101, action="begin", frame=0)])

    def test_invalid_events_parity(self):
        self.compare(setup_events() + [event(102, id="ram"), op("a"), op("a"),
            (103, b"\xff"), (102, b"\x01\x01\x00\xff"), (104, b"\x01x"),
            (101, b"\x01\xff"), (199, b"")])

    def test_utf8_and_trim_parity(self):
        self.compare([event(102, id="\u2003ram\u00a0", memory="ram"),
            event(102, id="vram", memory="vram"), op(" spaced ", source="\t ram \n"),
            op("second", source="ram"), event(102, action="release", id=" ram "), op("released")])

    def test_batch_parity_and_partial_rejection(self):
        batch = dict(batch_id="aabbccdd" * 4, operation_count=128, operation_type="upload",
                     source="ram", target="vram", queue="copy", size_bytes=4096, cost_us=300)
    ...
    def test_randomized_operation_parity(self):
        ...
        events = setup_events()
        for i in range(300):
            events.append(op(str(i), operation_type=rng.choice(["copy", "upload", "allocate", "draw", "compute", "sync"]),
                source=rng.choice(["ram", "vram", "missing"]), target=rng.choice(["ram", "vram", "missing"]),
                queue=rng.choice(["copy", "graphics"]), frame=rng.randrange(3),
                reason=rng.choice(["normal", "transient"]), size_bytes=rng.choice([0, 1024, 2048]),
    ...
    def test_network_authorization_and_identity(self):
        with server(executable=NATIVE) as (_, port), Peer(port) as peer:
            self.assertEqual(wire.decode_welcome_payload(peer.welcome.payload).server_name, "fluidgateway")
            peer.initialize()
    ...
    def test_header_rejections(self):
        ...
        for offset, value in ((0, 0), (4, 1), (5, 3), (9, 1), (10, 1)):
            data = bytearray(original); data[offset] = value; invalid.append(data.hex())
    ...
    def test_sequence_and_session_rejected(self):
        with server(executable=NATIVE) as (_, port), Peer(port) as peer:
            ...
            request = wire.fluidlink_v2_request(opcode=20, sequence=2, payload=b"\x01x", session_id=peer.session)
    ...
    def test_mutated_frame_codec_matches_reference(self):
        ...
        for _ in range(3000):
            ...
            try:
                expected.append(wire.encode_fluidlink_v2_frame(wire.decode_fluidlink_v2_frame(bytes(data))).hex())
    ...
    def test_frame_history_does_not_grow_retained_state(self):
        hello = wire.fluidlink_v2_request(opcode=1, sequence=1,
            payload=wire.encode_hello_payload(client_name="test", client_version="1"))
        requests = [wire.encode_fluidlink_v2_frame(hello).hex()]
        empty = subprocess.run([str(TEST_BINARY), "--replay"], input=requests[0],
            text=True, capture_output=True, check=True, timeout=10)
        for frame in range(10000):
            for action in ("begin", "end"):
                requests.append(wire.encode_fluidlink_v2_frame(wire.fluidlink_v2_request(
                    opcode=10, sequence=len(requests) + 1, session_id=IDENTITY, subject_opcode=101,
                    payload=wire.encode_runtime_event_payload(101, dict(action=action, frame=frame)))).hex())
        result = subprocess.run([str(TEST_BINARY), "--replay"], input="\n".join(requests),
        ...
        with server(executable=NATIVE) as (_, port):
            with Peer(port) as peer:
                for index in range(4096):
                    response = peer.event(102, id=f"r{index}", aliases=[f"{j:03d}" + "x" * 253 for j in range(32)])
    ...
    def test_integer_wire_units_are_exact_beyond_double_precision(self):
        with server(executable=NATIVE) as (_, port), Peer(port) as peer:
            peer.initialize()
            peer.event(103, id="a", operation_type="upload", source="ram", target="vram", size_bytes=2**53)
            changed = peer.event(103, id="b", operation_type="upload", source="ram", target="vram", size_bytes=2**53 + 1)
            self.assertEqual(changed.decision_opcode, 0)
            maximum = dict(operation_type="upload", source="ram", target="vram", size_bytes=2**64 - 1,
                           cost_us=2**32 - 1)
    ...
    def test_dense_alias_invalidation_at_resource_limit(self):
        requests = [wire.fluidlink_v2_request(opcode=1, sequence=1,
            payload=wire.encode_hello_payload(client_name="dense-alias-test", client_version="1"))]
        events = [event(102, id=str(i), memory="ram", aliases=["shared"]) for i in range(4095)]
        events += [event(102, id="vram", memory="vram"),
            op("seed", source="0"), op("duplicate", source="0"),
            op("write", operation_type="draw", target="4094"), op("fresh", source="0")]
        requests.extend(wire.fluidlink_v2_request(opcode=10, sequence=i, session_id=IDENTITY,
            subject_opcode=subject, payload=payload) for i, (subject, payload) in enumerate(events, 2))
        result = subprocess.run([str(TEST_BINARY), "--replay"], text=True, capture_output=True,
            input="\n".join(wire.encode_fluidlink_v2_frame(r).hex() for r in requests),
            check=True, timeout=10)
        responses = [wire.decode_fluidlink_v2_frame(bytes.fromhex(line)) for line in result.stdout.splitlines()]
